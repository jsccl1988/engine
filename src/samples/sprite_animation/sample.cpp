...
int main() {
  ...
  AnimatedSprite* walkingGuy = new AnimatedSprite(engine, 4);
  ...
  /* Generate frames */
  for (int i = 0; i < 4; i++) {
    walkingGuy->genQuadFrames(i, Vector2f(0, 288.0f - (i + 1) * dv), Vector2f(du, dv), Vector2f(du, 0), 4);
  }
  ...
  while (!finish) {
    ...
    walkingGuy->move(move * timer->getElapsedTime());
    timer->reset();
